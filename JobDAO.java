...
public class JobDAO {
    public static int addJob(Job j) throws SQLException {
        ...
        {
            ps.setInt(1, j.employerId);
            ps.setString(2, j.title);
            ps.setString(3, j.jd);
            ps.setString(4, j.pay);
            ps.setString(5, j.type);
            ps.executeUpdate();
    ...
    public static List<Job> searchByTitle(String keyword) throws SQLException 
    {
            ...
            PreparedStatement ps = c.prepareStatement(sql)) 
        {
            ...
            try (ResultSet rs = ps.executeQuery()) 
            {
                while (rs.next()) 
                {
                    list.add(new Job(rs.getInt("jobid"), rs.getInt("employer_id"), rs.getString("title"), rs.getString("jd"), rs.getString("pay"), rs.getString("type")));
                }
            }
        }
        return list;
    ...
    public static Job getById(int id) throws SQLException 
    {
        ...
        try (Connection c = DBConnection.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) 
        {
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return new Job(rs.getInt("jobid"), rs.getInt("employer_id"), rs.getString("title"), rs.getString("jd"), rs.getString("pay"), rs.getString("type"));
            }
        }
        return null;
    }
